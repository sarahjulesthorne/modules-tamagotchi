...
const quadDomStringBuilder = (arrayToPrint, divToDisplay) => {
  ...
  arrayToPrint.forEach((quadrant) => {
    if (quadrant.quadDivId === divToDisplay) {
      domString += `<div class="padding quadrant-title-container  ${quadrant.quadDivId}-quadrant-title-container" id="${quadrant.quadDivId}QuadrantTitleContainer">`;
      domString += `<h3 class="centered padding quadrant-title ${quadrant.quadDivId}-quadrant-title" id="${quadrant.quadDivId}QuadrantTitle">${quadrant.quadName}</h3>`;
      domString += `<p class="centered padding quadrant-score ${quadrant.quadDivId}-score" id="${quadrant.scoreDivId}">${quadrant.quadScoreValue}</p>`;
      domString += '</div>';
      domString += `<div class="padding ${quadrant.quadDivId}-button-container quadrant-button-container" id="${quadrant.quadDivId}ButtonContainer">`;
      quadrant.quadrantButtons.forEach((button) => {
        domString += `<div class="padding button-container ${quadrant.quadDivId}-button-container" id="${quadrant.buttonId}Container">`;
...
const headerDomStringBuilder = () => {
  let domString = '';
  domString = '<h1 class="page-title padding centered">My Tamagotchi</h1>';
